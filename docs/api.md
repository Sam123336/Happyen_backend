...
- `GET /v1/events/nearby?lat=&lng=&radius_m=&category=&starts_after=&starts_before=&limit=` — published event occurrences within `radius_m` metres of a point, soonest first. `radius_m` defaults to 5000 (max 50000), `limit` to 50 (max 200), and `starts_after` to now. `category` is one of `music`, `comedy`, `food`, `pets`, `sports`, `other`. Each result carries the occurrence id, its event id, title, category, venue name, coordinates, start/end times, hero image and distance in metres.
